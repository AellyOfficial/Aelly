            ...
            async (events) => {
                if (events['connection.update']) {
                    ...
                    if (connection === 'close') {
                        if ((lastDisconnect.error as Boom)?.output?.statusCode !== DisconnectReason.loggedOut) {
                            ...
                            console.log(chalk.redBright('Connection closed. You are logged out. Delete the Aelly.db and session.data.json files to rescan the code.'));
                            process.exit(0);
                        }
                    } else if (connection === 'connecting') {
                        console.log(chalk.yellowBright("[INFO] Connecting to WhatsApp..."));
                    } else if (connection === 'open') {
                        console.log(chalk.greenBright.bold("[INFO] Connected! Welcome to Aelly"));
                    }
                }
                ...
                if (events['messages.upsert']) {
                    ...
                    for(const msg of upsert.messages){
                        ...
                        let Aelly: Aelly = await resolve(chat, sock);
                        // console.log(Aelly);
                        if (Aelly.isCmd) {
                            let isBlacklist: boolean = await Blacklist.getBlacklistUser(Aelly.sender, Aelly.chatId);
                            const cleared: boolean = await clearance(Aelly, client, isBlacklist);
                            if (!cleared) {
                                return;
                            }
                            ...
                            console.log(chalk.redBright.bold(`[INFO] ${Aelly.commandName} command executed.`));
                            const command = commandHandler.get(Aelly.commandName);
                            var args = Aelly.body.trim().split(/\s+/).slice(1);
                            if (!command) {
                                client.sendMessage(Aelly.chatId, "```Woops, invalid command! Use```  *.help*  ```to display the command list.```", MessageType.text);
                                return;
                            } else if (command && Aelly.commandName == "help") {
                                try {
                                    command.handle(client, chat, Aelly, args, commandHandler);
                                    return;
                                } catch (err) {
                            ...
                            try {
                                await command.handle(client, chat, Aelly, args).catch(err => console.log("[ERROR] " + err));
                            } catch (err) {
                                console.log(chalk.red("[ERROR] ", err));
                            }
                        }
